...
class JavaManager:
    ...
    def _get_java_download_info(self, major_version: int):
        java_sources = {
            ...
            11: (
                "https://github.com/adoptium/temurin11-binaries/releases/download/jdk-11.0.23%2B9/OpenJDK11U-jre_x64_windows_hotspot_11.0.23_9.zip",
                "jdk-11.0.23+9",
            ),
            17: (
    ...
    def check_java_version(self, java_path: str, required_major: int) -> bool:
        try:
            ...
            if (
                f'version "{required_major}.' in version_output
                or f'version "{required_major}' in version_output
            ):
                return True
            if required_major == 8 and ("1.8." in version_output):
                return True
            if (
                f'"{required_major}.' in version_output
                or f'"{required_major}+' in version_output
            ):
                return True
    ...
    def download_portable_java(self, major_version=21, progress_callback=None) -> str:
        target_runtime_dir = os.path.join(
            self.runtimes_base_dir, f"java_{major_version}"
        )
        ...
        if os.path.exists(java_exe) and self.check_java_version(
            java_exe, major_version
        ):
            return java_exe
        ...
        if progress_callback:
            progress_callback(
                "Налаштування Java",
                f"Завантаження портативного OpenJDK {major_version}...",
                0.01,
            )
        ...
        try:
            ...
            if progress_callback:
                progress_callback(
                    "Налаштування Java", f"Розпаковка Java {major_version}...", 0.02
                )
    ...
    def detect_java_path(self, required_major=21, progress_callback=None) -> str:
        target_runtime_dir = os.path.join(
            self.runtimes_base_dir, f"java_{required_major}"
        )
        for exe_name in ["javaw.exe", "java.exe"]:
            local_path = os.path.join(target_runtime_dir, "bin", exe_name)
            if os.path.exists(local_path) and self.check_java_version(
                local_path, required_major
            ):
                return local_path
        ...
        if java_home:
            for exe_name in ["javaw.exe", "java.exe"]:
                ...
                if os.path.exists(jh_path) and self.check_java_version(
                    jh_path, required_major
                ):
                    return jh_path
        ...
        for cmd in ["javaw", "java"]:
            ...
            if (
                which_path
                and os.path.exists(which_path)
                and self.check_java_version(which_path, required_major)
            ):
                return which_path
        ...
        possible_dirs = [
            ...
            os.path.join(
                user_profile, "AppData", "Local", "Programs", "Eclipse Adoptium"
            ),
        ]
